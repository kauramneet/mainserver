        ...
        if (!!userData) {
            ...
            if (bcrypt.compareSync(formData.password, userData.password)) {
                ...
                if(userData.userType == 3)
                {
                    ...
                    const cdata = await restModel.findOne({userId:userData._id}).exec()
                    console.log(cdata)
                    res.send({ success: true, status: 200, message: "Login Successful", token: token, data: cdata,userdata:userData })
                }
                else
                {
                    // console.log("bye")
